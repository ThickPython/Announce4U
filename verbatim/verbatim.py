...
COMMAND_DESCRIPTIONS = [
    ...
    ('branch add/remove `server ID` `path name`', ("Add or remove Paths on your server. This is required to start adding branches.")),
...
PATH_HELP = [
    ('path add `pathname`', ("Creates a Path with a unique name to the current server")),
    ('path remove `pathname`', ("Removes a Path from the server, note there is no confirmation")),
]
...
async def on_message(message):
    ...
    if header == f'{summon}path':
        ...
        if path_command == "add" or path_command == "remove":
            if len(the_message) > 3 or len(the_message) < 3: 
                await channel.send(f"You either have too many variables or too little variables, use {summon}path help to view proper usage.")
                return
    ...
    if header == f'{summon}whitelist':
        ...
        if whitelist_command == "add" or whitelist_command == "remove":
            ...
            #checks
            if str_guild not in white_list or white_list[str_guild] == {}:
                await channel.send(f"You haven't added anyone to the whitelist yet! Use `{summon}whitelist @user` to whitelist a user")
                return
            #---
